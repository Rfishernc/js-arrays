...
function voterRegistrationStringBuilder() {
    const newString = `<a href='https:/${elizabethSanger.voterRegistrationUrl}' target='_blank'>Register Here</a>`;
    printToDom(newString, 'voterRegistration');
}

function donationFormStringBuilder() {
...
function statementStringBuilder() {
    let newString = '';
...
function volunteerStringBuilder() {
  let newString = '';
  for(let i = 0; i < elizabethSanger.volunteers.length; i++) {
    newString +=  `<div class='volunteer'>`
    newString +=    `<p>${elizabethSanger.volunteers[i].name}</p>`
...
function missionStringBuilder() {
  let newString = `<p>${elizabethSanger.missionStatement}</p>`;
  printToDom(newString, 'missionStatement');
}

function districtStringBuilder() {
  let newString = `<p>${elizabethSanger.congressionalDistrict}</p>`;
  printToDom(newString, 'congressionalDistrict');
}

function eventStringBuilder() {
  let newString = '';
  for(let i = 0; i < elizabethSanger.events.length; i++) {
    newString +=  `<div class='eventB'>`
    newString +=    `<p>${elizabethSanger.events[i].title}</p>`
